...
def load_profile(target: Path) -> dict:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("profile:"):
            ...
            profile["adopted"] = line.split(":", 1)[1].strip().strip("\"'").lower() == "true"
        elif line.startswith("standards_version:"):
            profile["standards_version"] = line.split(":", 1)[1].strip().strip("\"'")
...
def adoption_blockers(target: Path, profile: dict) -> list[str]:
    """Reasons this repo must not be written to yet.

    The presence of .standards.yml was previously treated as consent, and it is
    not: a repo can declare a profile long before anyone agrees to have three
    top-level files overwritten. The first sync into an established repository
    is destructive — AGENTS.md, CLAUDE.md and copilot-instructions.md are
    replaced wholesale, and a repo with a substantial hand-written CLAUDE.md
    loses it.

    Two independent gates, because either alone fails open:

      - `sync.adopted: true` is the owner's explicit statement of consent.
    ...
    if not profile.get("adopted"):
        blockers.append(
            ".standards.yml does not set `sync.adopted: true`. The first sync "
            "overwrites AGENTS.md, CLAUDE.md and .github/copilot-instructions.md, "
...
def main() -> int:
    ...
    parser.add_argument("--adopt", action="store_true",
                        help="override the first-sync consent checks (see adoption_blockers)")
    ...
    if blockers and not args.adopt:
        ...
        print("\nRun with --adopt to override, after reading what it would replace.",
              file=sys.stderr)
        return 3

    pinned = profile["standards_version"]
    if pinned and pinned != standards_version().split("-")[0]:
        print(f"warning: {target.name} pins standards {pinned}, but this tree is "
              f"{standards_version()}. Sync from the pinned tag instead.", file=sys.stderr)
